...
function MyFavoriteStations() {
    const { myFavoriteStations, username, token } = useSelector((state) => state.userReducer);
    const dispatch = useDispatch();

    console.log(token)

    return (
        !username && myFavoriteStations ? <Loader /> : <div className='h-screen max-h-[73%] tablet-landscape:max-h-[92vh] w-screen tablet-landscape:w-2/5  tablet-landscape:mx-auto mt-[8.9rem] tablet-landscape:mt-[5em] tablet-landscape:[100vh] overflow-scroll'>
            <Station />
            {
                                ...
                                <div className='flex items-center'>
                                    ...
                                    <div className='flex justify-between'>
                                        <div className='w-full'>
                                            <h1 className='w-full mb-1 text-white line-clamp-1'>{station?.station_name}</h1>
                                            <span className='w-full text-sm font-light line-clamp-1'> {station?.city}, {station?.street_address}</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
